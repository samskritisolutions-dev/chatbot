...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-8 bg-gradient-to-r from-blue-600/20 to-purple-600/20 rounded-3xl border border-blue-500/20 shadow-2xl shadow-blue-500/10">
        ...
        <div className="flex bg-black/40 p-1.5 rounded-2xl border border-white/5 self-start md:self-center">
          <button 
            onClick={() => setActiveTab('overview')}
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'overview' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-gray-400 hover:text-white'}`}
          >
            <LayoutDashboard size={18} /> Overview
          </button>
          <button 
            onClick={() => setActiveTab('conversations')}
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'conversations' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-gray-400 hover:text-white'}`}
          >
      ...
      {activeTab === 'overview' ? (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <StatCard title="Total Clients" value={stats?.total_clients} icon={<Users className="text-blue-400" />} trend="+12% this month" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 bg-[#161618] rounded-3xl border border-white/5 overflow-hidden shadow-xl">
              <div className="p-6 border-b border-white/5 flex items-center justify-between">
                <h3 className="font-bold text-lg">Client Management</h3>
                <div className="flex items-center gap-2">
                  <div className="relative">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={14} />
                    <input type="text" placeholder="Search clients..." className="bg-white/5 border border-white/10 rounded-lg py-1.5 pl-9 pr-4 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500" />
                  </div>
                </div>
              </div>
                      ...
                      <tr key={client.id} className="hover:bg-white/[0.02] transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-lg bg-blue-600/10 flex items-center justify-center text-blue-400 font-bold text-xs uppercase">{client.name.substring(0, 2)}</div>
                            <div>
                              <p className="text-sm font-semibold">{client.name}</p>
                              <p className="text-[10px] text-gray-500 uppercase tracking-tighter">{client.email}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <select 
                            ...
                            className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500 capitalize"
                          >
            ...
            <div className="bg-[#161618] rounded-3xl border border-white/5 overflow-hidden flex flex-col shadow-xl">
                ...
                {stats?.top_bots.map((bot) => (
                  <div key={bot.id} className="flex items-center gap-4 group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-500 uppercase tracking-wider">{bot.conversations_count} total messages</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs font-bold text-emerald-400">92%</p>
                      <p className="text-[10px] text-gray-500 uppercase tracking-tighter">Health</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="bg-[#161618] rounded-3xl border border-white/5 overflow-hidden shadow-xl">
          <div className="p-6 border-b border-white/5">
            ...
            <p className="text-xs text-gray-500">Real-time view of all interactions across the platform.</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
              ...
              <tbody className="divide-y divide-white/5">
                {conversations.map((conv) => (
                  <tr key={conv.session_id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="px-6 py-4">
                      <p className="text-sm font-semibold">{conv.lead?.name || `Visitor ${conv.session_id.substring(0, 8)}`}</p>
                      <p className="text-[10px] text-gray-500 uppercase tracking-tighter">{conv.lead?.email || 'Anonymous'}</p>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-[10px] text-gray-500 uppercase">{new Date(conv.last_message_at).toLocaleTimeString()}</p>
                    </td>
      ...
      {selectedSession && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 lg:p-8">
          ...
          <div className="relative w-full max-w-2xl bg-[#0c0c0e] rounded-3xl border border-white/10 shadow-2xl flex flex-col max-h-[90vh] animate-slide-up">
            <div className="p-6 border-b border-white/5 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-lg">Transcript Review</h3>
                <p className="text-[10px] text-gray-500 uppercase tracking-widest">Session: {selectedSession}</p>
              </div>
              ...
              {loadingMessages ? (
                ...
                sessionMessages.map((msg, i) => (
                  <div key={i} className={`flex items-start gap-3 ${msg.role === 'user' ? '' : 'flex-row-reverse text-right'}`}>
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${msg.role === 'user' ? 'bg-white/5 text-gray-400' : 'bg-blue-600/20 text-blue-400'}`}>
                      {msg.role === 'user' ? <Users size={14} /> : <Bot size={14} />}
                    </div>
                    <div className={`max-w-[80%] p-3 rounded-2xl text-xs ${msg.role === 'user' ? 'bg-white/5 text-gray-300' : 'bg-blue-600 text-white'}`}>
                      {msg.message}
                    </div>
                  </div>
                ))
            ...
            <div className="p-6 bg-white/5 border-t border-white/5 rounded-b-3xl">
              <p className="text-[10px] text-center text-gray-500 uppercase tracking-widest">End of Transcript</p>
            </div>
          </div>
        </div>
      )}
...
function StatCard({ title, value, icon, trend }) {
  return (
    <div className="bg-[#161618] p-6 rounded-3xl border border-white/5 hover:border-white/10 transition-all group shadow-xl">
      <div className="flex items-center justify-between mb-4">
        <div className="p-3 bg-white/5 rounded-2xl group-hover:scale-110 transition-transform">{icon}</div>
        <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{trend}</div>
      </div>
      <p className="text-xs text-gray-500 font-medium mb-1 uppercase tracking-tighter">{title}</p>
